...
@section('page-title', 'Modifier : ' . $produit->nom)
...
{{-- ✅ FORMULAIRE PRINCIPAL (avec id, SANS formulaires imbriqués) --}}
<form id="form-modifier-produit" method="POST" action="{{ route('admin.produits.mettre-a-jour', $produit->id) }}" enctype="multipart/form-data" class="space-y-6">
    @csrf
    @method('PUT')
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8">
                    ...
                    @foreach ($produit->images as $img)
                        <div class="relative group">
                            ...
                            @if ($img->est_principale)
                                <span class="absolute top-2 left-2 bg-nissa-rose text-white text-xs px-2 py-1 rounded-full">⭐ Principale</span>
                            @else
                                {{-- ✅ Bouton lié au formulaire séparé --}}
                                <button type="submit" form="form-img-principale-{{ $img->id }}"
                                    class="absolute top-2 left-2 bg-white/90 text-xs px-2 py-1 rounded-full hover:bg-white">⭐</button>
                            @endif
                            {{-- ✅ Bouton lié au formulaire séparé --}}
                            <button type="submit" form="form-img-suppr-{{ $img->id }}"
                                class="absolute top-2 right-2 bg-red-500 text-white w-7 h-7 rounded-full flex items-center justify-center hover:bg-red-600">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
                <label class="block text-sm font-medium text-nissa-choco mb-2">Nom <span class="text-nissa-rose">*</span></label>
            ...
            <div>
                ...
                <input type="number" name="prix_base" value="{{ old('prix_base', $produit->prix_base) }}" required min="0"
                    class="w-full px-4 py-3 border border-gray-200 rounded-2xl focus:outline-none focus:border-nissa-rose transition">
            </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8" x-data="variantesBuilder()">
        ...
        <p class="text-sm text-gray-500 mb-6">
            💡 Modifiez les variantes existantes ou ajoutez-en de nouvelles. Les nouvelles couleurs seront créées automatiquement.
        </p>
                ...
                <div class="bg-nissa-cream rounded-2xl p-5 border border-gray-100">
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center gap-3">
                            ...
                            <template x-if="variante.id">
                                <span class="text-xs text-gray-400 bg-gray-100 px-2 py-0.5 rounded">existante</span>
                            </template>
                        </div>
                    ...
                    <input type="hidden" :name="`variantes[${vIndex}][id]`" x-model="variante.id">
...
{{-- ✅ FORMULAIRES SÉPARÉS (hors du formulaire principal, pour les images) --}}
@foreach ($produit->images as $img)
    <form id="form-img-principale-{{ $img->id }}" method="POST" action="{{ route('admin.images.principale', $img->id) }}" class="hidden">
        @csrf
        @method('PATCH')
    </form>
    <form id="form-img-suppr-{{ $img->id }}" method="POST" action="{{ route('admin.images.supprimer', $img->id) }}" class="hidden">
        @csrf
        @method('DELETE')
    </form>
@endforeach
...
document.querySelectorAll('form[id^="form-img-suppr-"]').forEach(function (f) {
