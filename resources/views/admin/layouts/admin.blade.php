...
<head>
    ...
    <link href="https://fonts.bunny.net/css?family=poppins:300,400,500,600,700|playfair-display:400,600,700" rel="stylesheet" />
    ...
    <div x-data="{ sidebarOpen: false, showModal: false, modalMessage: '', modalAction: null }">
        ...
        <aside id="admin-sidebar"
            ...
            <div class="p-6 border-b border-white/10 shrink-0">
                <h1 class="text-xl font-semibold tracking-wide" style="font-family: 'Playfair Display', serif;">NISSA</h1>
            ...
            <div class="p-4 border-t border-white/10 shrink-0">
                <div class="flex items-center gap-3 mb-3">
                    <div class="w-9 h-9 rounded-full bg-nissa-rose flex items-center justify-center text-white font-semibold text-sm shrink-0">
                ...
                <form method="POST" action="{{ route('admin.deconnexion') }}">
                    @csrf
                    <button type="submit" class="w-full text-left px-4 py-2 rounded-xl text-sm text-white/80 hover:bg-white/10 transition flex items-center gap-2">
        ...
        <div class="lg:pl-64 flex flex-col min-h-screen">
            ...
            <header class="bg-white border-b border-gray-200 sticky top-0 z-20 shrink-0">
                <div class="flex items-center justify-between px-4 sm:px-6 py-3">
                    <div class="flex items-center gap-3">
                        <button @click="sidebarOpen = !sidebarOpen" class="lg:hidden w-10 h-10 flex items-center justify-center rounded-xl hover:bg-gray-100 transition">
                    ...
                    <div class="flex items-center gap-3">
                        <a href="{{ route('accueil') }}" target="_blank" class="hidden sm:flex items-center gap-2 text-sm text-gray-600 hover:text-nissa-rose transition">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                            Voir le site
                        </a>
                        <div class="w-px h-6 bg-gray-200 hidden sm:block"></div>
                        <div class="flex items-center gap-2">
                            <div class="w-9 h-9 rounded-full bg-nissa-rose text-white flex items-center justify-center font-semibold text-sm">
                                {{ strtoupper(substr(Auth::guard('admin')->user()->nom, 0, 1)) }}
                            </div>
                            <div class="hidden md:block">
                                <p class="text-sm font-medium leading-tight">{{ Auth::guard('admin')->user()->nom }}</p>
            ...
            <div x-show="showModal" x-transition class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" style="display: none;">
                <div class="bg-white rounded-3xl shadow-2xl max-w-md w-full p-8">
                    <div class="text-center">
                        ...
                        <div class="flex gap-3">
                            <button @click="showModal = false" class="flex-1 px-6 py-3 border-2 border-gray-200 text-gray-700 rounded-2xl font-semibold hover:bg-gray-50 transition">
                                Annuler
                            </button>
                            <button @click="if(modalAction) modalAction.submit(); showModal = false;" class="flex-1 px-6 py-3 bg-red-500 text-white rounded-2xl font-semibold hover:bg-red-600 transition">
                                Confirmer
                            </button>
                        </div>
                    </div>
