    ...
    @foreach ([
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <form method="POST" action="{{ route('admin.attributs.enregistrer') }}" class="flex gap-2 mb-5">
                ...
                {{-- ✅ Color picker uniquement pour les couleurs --}}
                @if ($section['cle'] === 'couleur')
                    <input type="color" name="code_hex" value="#D98B92" 
                        class="w-12 h-11 rounded-xl border border-gray-200 cursor-pointer p-1">
                @endif
                
                <button type="submit"
                    class="px-4 py-2.5 bg-nissa-choco text-white rounded-xl text-sm font-semibold hover:bg-nissa-rose transition">+</button>
            </form>
                ...
                @forelse ($section['liste'] as $item)
                    <div class="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
                        <div class="flex items-center gap-2">
                            {{-- ✅ Afficher le carré de couleur pour les couleurs --}}
                            @if ($section['cle'] === 'couleur' && $item->code_hexadecimal)
                                <div class="w-6 h-6 rounded-full border border-gray-200" 
                                     style="background-color: {{ $item->code_hexadecimal }}"></div>
                            @endif
                            <span class="text-sm font-medium text-nissa-choco">{{ $item->nom }}</span>
                        </div>
                        <div class="flex items-center gap-2">
                            <form method="POST"
                                action="{{ route('admin.attributs.toggle', [$section['cle'], $item->id]) }}">
                                @csrf
                                @method('PATCH')
                                <button type="submit"
                                    class="px-2.5 py-1 text-xs font-medium rounded-full {{ $item->actif ? 'bg-green-100 text-green-800' : 'bg-gray-200 text-gray-600' }}">
                                    {{ $item->actif ? 'Actif' : 'Inactif' }}
                                </button>
                            </form>
                            <form method="POST"
                                action="{{ route('admin.attributs.supprimer', [$section['cle'], $item->id]) }}"
                                @submit.prevent="modalMessage = 'Supprimer cet élément ?'; modalAction = $event.target; showModal = true">
                                @csrf
                                @method('DELETE')
