...
<div class="mb-8">
    <h1 class="text-3xl font-bold text-nissa-choco" style="font-family: 'Playfair Display', serif;">Catégories</h1>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sticky top-24">
            ...
            <form method="POST" action="{{ route('admin.categories.enregistrer') }}" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="nom" required placeholder="Ex: Chouchous, Sacs..."
    ...
    <div class="lg:col-span-2">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="divide-y divide-gray-100">
                    ...
                    <div class="p-5 flex items-center justify-between hover:bg-gray-50 transition">
                        <div class="flex items-center gap-4">
                            <div class="w-12 h-12 bg-nissa-rose/10 rounded-xl flex items-center justify-center">
                                <svg class="w-6 h-6 text-nissa-rose" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"/></svg>
                            </div>
                            <div>
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <form method="POST" action="{{ route('admin.categories.supprimer', $categorie->id) }}"
                                  @submit.prevent="modalMessage = 'Supprimer la catégorie {{ $categorie->nom }} ?'; modalAction = $event.target; showModal = true">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                    <div class="p-12 text-center text-gray-500">Aucune catégorie</div>
                @endforelse
            </div>
        </div>
    </div>
