    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div class="divide-y divide-gray-100">
                @foreach ($commande->articles as $article)
                    <div class="p-6 flex items-center gap-4">
                        @if ($article->pack)
                            <div class="w-16 h-16 bg-nissa-rose/10 rounded-xl flex items-center justify-center shrink-0">
                                <svg class="w-8 h-8 text-nissa-rose" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        @else
                            @php
                                $img = $article->produit->images->where('est_principale', true)->first() ?? $article->produit->images->first();
                            ...
                            <div class="w-16 h-16 bg-gray-100 rounded-xl overflow-hidden shrink-0">
                                @if ($cheminImg)
                                    <img src="{{ $cheminImg }}" alt="{{ $article->produit->nom }}" class="w-full h-full object-cover">
                                @endif
                            </div>
                            <div class="flex-1">
                        ...
                        <div class="text-right">
                            ...
                            <p class="font-bold text-nissa-rose">{{ number_format($article->prix_total, 0, ',', ' ') }} FCFA</p>
                        </div>
                    </div>
                @endforeach
            </div>
            <div class="px-6 py-4 bg-gray-50 flex justify-between items-center">
                ...
                <span class="text-2xl font-bold text-nissa-rose">{{ number_format($commande->total, 0, ',', ' ') }} FCFA</span>
            </div>
        </div>
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <div class="space-y-3">
                <div class="flex justify-between items-center">
                    ...
                    <span class="inline-flex px-2.5 py-1 text-xs font-medium rounded-full {{ $couleursStatut[$commande->statut] ?? 'bg-gray-100' }}">
                        {{ $labelsStatut[$commande->statut] ?? $commande->statut }}
                    </span>
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-sm text-gray-500">Paiement</span>
                    <span class="inline-flex px-2.5 py-1 text-xs font-medium rounded-full {{ $couleursPaiement[$commande->statut_paiement] ?? 'bg-gray-100' }}">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <form method="POST" action="{{ route('admin.commandes.updateStatut', $commande->id) }}" class="space-y-4">
                ...
                <div>
                    ...
                    <select name="statut_paiement" class="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-nissa-rose">
                ...
                <button type="submit" class="w-full bg-nissa-choco text-white py-2.5 rounded-xl font-semibold hover:bg-nissa-rose transition">
                    Mettre à jour
                </button>
            </form>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <div class="space-y-2 text-sm">
                <p class="text-gray-700"><strong>{{ $commande->cliente->prenom ?? '' }} {{ $commande->cliente->nom ?? '' }}</strong></p>
            ...
            @php
                ...
                $messageClient .= "Nous préparons votre commande avec soin. Nous vous tiendrons informée de l'avancement.\n\n";
