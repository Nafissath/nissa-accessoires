...
<section class="py-12 md:py-16 bg-[#FDFBF7]">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {{-- Titre Épuré & Discret --}}
        <div class="flex flex-col md:flex-row md:items-end justify-between mb-10 pb-6 border-b border-[#E8A598]/20 gap-4">
            <div>
                <span class="inline-block text-xs font-semibold uppercase tracking-[0.25em] text-[#E8A598] mb-1">
                    Offres Spéciales
                </span>
                <h1 class="text-3xl md:text-4xl font-bold text-[#4A3525]" style="font-family: 'Playfair Display', serif;">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                @foreach ($packs as $pack)
                    <a href="{{ route('packs.afficher', $pack->slug) }}" class="group block">
                        <article class="bg-white rounded-3xl border border-[#F9EBEA] overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col justify-between h-full">

                            {{-- Visuel du Pack --}}
                            <div class="relative aspect-[4/3] bg-[#FDFBF7] overflow-hidden">
                                @php
                                    $premierArticle = $pack->articles->first();
                                    $imagePrincipale = $premierArticle && $premierArticle->produit
                                        ? ($premierArticle->produit->images->where('est_principale', true)->first() ?? $premierArticle->produit->images->first())
                                        : null;
                                ...
                                <div class="absolute top-4 left-4 flex flex-col gap-2">
                                    <span class="bg-[#4A3525]/90 backdrop-blur text-white px-3 py-1 rounded-full text-[11px] font-medium tracking-wide shadow-sm">
                                ...
                                @if ($pack->prix_promo)
                                    ...
                                    @endphp
                                    <span class="absolute top-4 right-4 bg-[#E8A598] text-white px-3 py-1 rounded-full text-[11px] font-bold shadow-sm">
                            ...
                            <div class="p-6 flex flex-col justify-between flex-1">
                                ...
                                <div class="pt-4 border-t border-[#F9EBEA] flex items-center justify-between">
                                    ...
                                    <span class="w-9 h-9 rounded-full bg-[#F9EBEA] group-hover:bg-[#4A3525] group-hover:text-white text-[#4A3525] flex items-center justify-center transition">
        ...
        @else
            <div class="text-center py-20 bg-white rounded-3xl border border-[#F9EBEA] space-y-4">
                <div class="w-16 h-16 mx-auto rounded-full bg-[#F9EBEA] flex items-center justify-center text-[#E8A598]">
                ...
                <a href="{{ route('boutique') }}" class="inline-block bg-[#4A3525] text-white px-6 py-2.5 rounded-full text-xs font-bold hover:bg-[#E8A598] transition">
