            ...
            <div>
                ...
                <div class="relative aspect-square bg-white rounded-3xl overflow-hidden shadow-xl">
                    ...
                    @if ($pack->prix_promo)
                        ...
                        <span class="absolute top-5 right-5 bg-[#E8A598] text-white px-4 py-1.5 rounded-full text-xs font-bold shadow-lg">
                ...
                <div class="bg-white rounded-2xl border border-[#F9EBEA] p-6 mb-8">
                    ...
                    <ul class="space-y-3">
                        @foreach ($pack->articles as $article)
                            @php
                                $imgArt = $article->produit->images->where('est_principale', true)->first() ?? $article->produit->images->first();
                                $cheminImgArt = $imgArt
                                    ? (str_starts_with($imgArt->chemin, 'http') ? $imgArt->chemin : asset('storage/' . $imgArt->chemin))
                                    : null;
                            @endphp
                            <li class="flex items-center gap-4">
                                ...
                                <div class="flex-1">
                                    ...
                                    <p class="text-xs text-gray-500">{{ $article->produit->categorie->nom ?? 'Accessoire' }}</p>
                                </div>
