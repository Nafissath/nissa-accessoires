        ...
        <div class="max-w-7xl mx-auto px-4">
                ...
                <div class="space-y-4">
                    <div class="aspect-[4/5] bg-gradient-to-br from-nissa-cream to-white rounded-3xl overflow-hidden shadow-xl">
                        ...
                        @if($imagePrincipale)
                            ...
                            @endphp
                            <img id="image-principale" src="{{ $cheminImage }}" alt="{{ $produit->nom }}" class="w-full h-full object-cover">
                        @else
                        ...
                        <div class="grid grid-cols-4 gap-3">
                            @foreach($produit->images as $img)
                                ...
                                <button onclick="document.getElementById('image-principale').src='{{ $cheminMini }}'"
                                    ...
                                    <img src="{{ $cheminMini }}" alt="{{ $produit->nom }}" class="w-full h-full object-cover">
                                </button>
                            @endforeach
                        </div>
                    @endif
                ...
                <div>
                    ...
                    @if($produit->nuances_couleurs)
                        <div class="flex items-center gap-2 mb-4">
                            ...
                            <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-nissa-cream border border-nissa-rose/30 rounded-full text-sm font-medium text-nissa-choco">
            ...
            <div class="mt-16 max-w-4xl" x-data="{ note: 0 }">
                    ...
                    <div class="flex items-center gap-4 mb-8 bg-[#FBF8F3] rounded-2xl p-5">
                        ...
                        <div>
                            <div class="flex gap-1">
                                @for($i = 1; $i <= 5; $i++)
                                    <svg class="w-5 h-5 {{ $i <= round($avis->avg('note')) ? 'text-nissa-gold' : 'text-gray-300' }}" fill="currentColor" viewBox="0 0 20 20">
                            ...
                            <div class="bg-white border border-gray-100 rounded-2xl p-5">
                                <div class="flex items-center justify-between mb-3">
                                    <div class="flex items-center gap-3">
                                        <div class="w-10 h-10 rounded-full bg-nissa-rose/10 flex items-center justify-center text-nissa-rose font-semibold">
                ...
                <div class="bg-white border border-gray-100 rounded-2xl p-6">
                    <h3 class="text-xl font-semibold text-nissa-choco mb-5" style="font-family: 'Playfair Display', serif;">
                        Laisser un avis
                    </h3>
                    ...
                    <form method="POST" action="{{ route('avis.store', $produit->slug) }}">
                        ...
                        <div class="mb-5">
                            ...
                            <div class="flex gap-1">
                                @for($i = 1; $i <= 5; $i++)
                                    <button type="button" @click="note = {{ $i }}" class="p-1">
                                        <svg class="w-8 h-8 transition-colors" :class="note >= {{ $i }} ? 'text-nissa-gold' : 'text-gray-300'" fill="currentColor" viewBox="0 0 20 20">
                                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                                        </svg>
                                    </button>
                                @endfor
                            ...
                            <p class="text-xs text-gray-400 mt-1" x-text="note === 0 ? 'Cliquez sur les étoiles pour noter' : note + ' / 5'"></p>
                        </div>

                        <div class="grid md:grid-cols-2 gap-4 mb-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Votre nom</label>
                                <input type="text" name="nom" required maxlength="100" class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-nissa-rose" placeholder="Ex : Aïcha K.">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Votre email</label>
                                <input type="email" name="email" required class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-nissa-rose" placeholder="votre@email.com">
                            </div>
                        </div>

                        <div class="mb-5">
                            <label class="block text-sm font-medium text-gray-700 mb-1">Votre commentaire</label>
                            <textarea name="commentaire" required rows="4" minlength="10" maxlength="1000" class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-nissa-rose" placeholder="Partagez votre expérience avec ce produit..."></textarea>
                        </div>

                        <button type="submit" class="bg-nissa-choco text-white px-6 py-3 rounded-xl text-sm font-semibold hover:bg-nissa-rose transition">
                            Envoyer mon avis
                        </button>
                            ...
                            <a href="{{ route('produit.afficher', $prod->slug) }}" class="group">
                                <div class="aspect-square bg-nissa-cream rounded-2xl overflow-hidden mb-3">
                                    @if($prod->images->first())
                                        ...
                                        @endphp
                                        <img src="{{ $cheminSim }}" alt="{{ $prod->nom }}" class="w-full h-full object-cover group-hover:scale-110 transition" loading="lazy">
                                    @else
                                        <div class="w-full h-full flex items-center justify-center">
