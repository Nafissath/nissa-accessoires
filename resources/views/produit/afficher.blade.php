...
@section('content')
        ...
        <div class="max-w-7xl mx-auto px-4">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
                ...
                <div>
                    ...
                    <!-- Prix -->
                    <div class="flex items-baseline gap-3 mb-6" x-data x-init="$dispatch('prix-initial', { base: {{ $produit->prix_base }}, promo: {{ $produit->prix_promo ?? 'null' }} })">
                        ...
                        @foreach ($produitsSimilaires as $prod)
                            <a href="{{ route('produit.afficher', $prod->slug) }}" class="group">
                                <div class="aspect-square bg-nissa-cream rounded-2xl overflow-hidden mb-3">
                                    ...
                                    @else
                                        <div class="w-full h-full flex items-center justify-center">
                                            <svg class="w-16 h-16 text-nissa-rose/40" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1"
                                                    d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z">
                                                </path>
    ...
    {{-- Script pour synchroniser le prix affiché avec Livewire --}}
    <script>
        document.addEventListener('livewire:initialized', () => {
            Livewire.on('prix-mis-a-jour', (data) => {
                const prixElt = document.getElementById('prix-affichage');
                if (prixElt && data.prix) {
                    prixElt.textContent = new Intl.NumberFormat('fr-FR').format(data.prix);
                }
            });
        });
    </script>
    ...
    <script type="application/ld+json">
    {
        ...
        "image": [
            @foreach ($produit->images as $img)
                "{{ str_starts_with($img->chemin, 'http') ? $img->chemin : asset('storage/' . $img->chemin) }}"{{ !$loop->last ? ',' : '' }}
            @endforeach
        ],
        ...
        "brand": {
            "@type": "Brand",
            "name": "Nissa Accessoires"
        },
        "offers": {
            "@type": "Offer",
            "url": "{{ route('produit.afficher', $produit->slug) }}",
            "priceCurrency": "XOF",
            "price": "{{ $produit->prix_promo ?? $produit->prix_base }}",
            "availability": "{{ $produit->variantes->sum('stock') > 0 ? 'https://schema.org/InStock' : 'https://schema.org/OutOfStock' }}",
            "seller": {
                "@type": "Organization",
                "name": "Nissa Accessoires"
            }
